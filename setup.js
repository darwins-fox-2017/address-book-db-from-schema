...
var CREATE_TABEL_CONTACS = "CREATE TABLE IF NOT EXISTS contacts(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, company TEXT NOT NULL ,email TEXT NOT NULL ,phone_number TEXT NOT NULL )"
var CREATE_TABEL_GROUPS = "CREATE TABLE IF NOT EXISTS groups(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL)"
var CREATE_TABEL_GROUPS_CONTACS = "CREATE TABLE IF NOT EXISTS groups_contacts (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER, contact_id INTEGER, FOREIGN KEY(group_id) REFERENCES groups (id), FOREIGN KEY(contact_id) REFERENCES contacts (id))"
...
let initData = (contactsFromJSONFile, groupsFromJSONFile, contactGroup) => {
  ...
  db.serialize(function(){
    for (let i = 0; i < contactsFromJSONFile.length; i++) {
      db.run(INSERT_CONTACT, [contactsFromJSONFile[i].name, contactsFromJSONFile[i].company, contactsFromJSONFile[i].email, contactsFromJSONFile[i].phone_number], function(err){
        if (err) {
          console.log(err);
        } else {
